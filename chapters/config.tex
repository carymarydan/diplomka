...
\paragraph{Aleph.ini}
Jelikož naším integrovaným knihovnickým systémem je Aleph, konfiguraèní soubor, který upøesòuje informace o tomto systému se jmenuje Aleph.ini. V pøípadì používání jiného ILS, napøíklad Koha, se konfiguraèní soubor pro nastavení upøesòujících informací o systému jmenuje Koha.ini. Je zde zadáno adresa, na které ILS Aleph bìží, tedy aleph.techlib.cz. Dále jméno báze, které je STK01. To je použito pøi vytváøení dotazu na X-server Alephu, pro získání informací o konkrétní knihovní jednotce. Jsou zde také uvedeny dvì zkratky PUJC a BHF, které znamenají Pùjèovna a Badatelna historického fondu a oznaèují místa vyzvednutí, kde lze po rezervaci knihovní jednotky oèekávat její výdej. V uživatelském rozhraní se však Pùjèovna zobrazuje jako Centrální pult, protože tak je ve druhém nadzemním podlaží reálnì oznaèen. Dále je zde obvykle uvedena cesta k umístìní souboru, který obsahuje pøevodní tabulky kódù umístìní jednotek, dílèích knihoven, statusù jednotek, atd., jež vrací na dotaz Aleph ve formátu XML spoleènì s dalšími informacemi, které nejsou v indexu VuFindu a musí se tak naèítat v reálném èase pøi prohlížení konkrétních záznamù v katalogu. V našem prostøedí je však naèítání tohoto souboru øešeno jiným zpùsobem. A to tak, že se soubor naèítá pøímo ze ovladaèe Alephu, kterým je:

/module/ntk_module/src/ntk_module/ILS/Driver/Aleph.php

Soubor s pøevodními tabulkamy se jmenuje AlephTables.php a je umístìn ve složce aleph_tab v koøenovém adresáøi.
