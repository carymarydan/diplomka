\label{sec:ntkmodule}
Jádrem portálu VuFind je složka \texttt{module} v koøenovém adresáøi systému. Protože pøední vlastností tohoto systému je modularita, je zde zdrojový kód rozøazen do modulù, dle jejich významu. Díky této vlastnosti lze do systému pøidávat vlastní moduly. Modulem pro prostøedí NTK je \texttt{ntk\_module}.
...
Obálky knih, které se v katalogu NTK zobrazují, se získávají buï pøímo v knihovnì pøi skenování, a nebo se získávají z portálu ObalkyKnih.cz. Aby tato logika fungovala v tomto poøadí, tedy zjistit, zda je obálka na lokálním serveru knihovny, popøípadì ji potom hledat na serveru Obalkyknih.cz je v objektovì orientovaném PHP naprogramována nová tøída \texttt{AlephImageCovers}, která vypadá takto:
\begin{lstlisting}
